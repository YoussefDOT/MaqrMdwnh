...
| Cinematic guard / background work / staged reveals | `sceneBusy`, `whenCalm`, `calmTurn`, `stageFrames`, `_lockInSteps` |
| Sprite density hysteresis + bake ration | `_sprUpdateLevel`, `_sprBakeBudget`, `SPR_BAKES_PER_FRAME` |
| Race track classification (worker) | `loadRaceTrackAsset`, `_raceTrackZonesInWorker`, `_raceTrackZones`, `_raceTrackAssemble` |
| HUD boxes ignoring the slide transform | `_hudLayoutRect`, `_hudScheduleDock` |
...
1. `node --input-type=module --check < game.js` — must pass. **Plain `node --check game.js`
   is NOT enough**: it let a stray `else` (a SyntaxError that killed the whole module in the
   browser) straight through. (For inline `<script>` edits, re-read the diff.)
...
31. **Nothing heavy that isn't urgent may land during a cinematic.** Post-spawn loads go
    through `whenCalm(fn)`, never a bare `requestIdleCallback` (its timeout and a 30 fps
    cap's idle gaps put every "idle" task INSIDE the entrance). A big continuation after a
    download (a JSON parse, a decode) awaits `calmTurn()`. A burst of DOM changes at a
    cinematic moment is spread with `stageFrames` (see **اللحظات المرئية**). Big pixel work
    goes to a worker (the race track), and an image drawn on the canvas is `decode()`d
    first — `drawImage` of an undecoded image decodes it on the main thread.
32. **Never reallocate the game canvas except through `resizeCanvas()`**, which carries the
    old picture across (setting `width` wipes it — the one-frame black flash). A governor
    resolution step is applied at the START of a frame (`_perfApplyRes`), never after a
    draw.
33. **Never a translucent TEXT colour** (`color: rgba(…, <1)`, an 8-digit hex, a var
    holding one, or a canvas `fillStyle` with alpha before `fillText`). Arabic letters
    overlap at their joins and each glyph is composited on its own, so every overlap is
    painted twice — a brighter seam between the letters. Use the SOLID colour it renders
    as (blend it yourself over its background: dark glass ≈ `#141416`, paper `#faf9f7`).
...
- **Race track** (`Art/RaceTrack Var1.png`, 6 MB + CPU-heavy pixel classification):
  ...
  held back in a hidden tab). No OffscreenCanvas → the old main-thread path, after
  `calmTurn()`. It used to land mid-entrance for every returning visitor.
- **"After spawn" means "once calm"**: `whenCalm` (see **اللحظات المرئية**) owns every
  post-spawn warm-up — sound warm-up, stickers, hats, prop art, Lemo's Play sheet, the
  library fetch, trophies, meeting art, boss sounds.
...
**The «هل تريد راحة؟» countdown is PERSISTED, not restarted.** `workMsAtLastBreak` is written into the free laptop doc, the reclaim stash and the cross-user stash, and both restore paths set it to **`stored mark + awayMs`** (clamped to the restored total). So work done before the tab closed still counts toward the 25 minutes — a member who had worked 20 of them comes back with 20, not 0 — while the credited away hours stay OUT of the threshold, which is what stops a 6-hour gap nagging them on the first frame back. **Don't pin the mark to the restored total** (that was the old behaviour and it silently wiped the countdown on every reload), and don't drop the `+ awayMs` shift. `breakPromptShown` is deliberately restored as **false**: the «لا» snooze is a JS timer that a reload kills, so a prompt already earned must be re-offered.
...
### 7b. اللحظات المرئية — cinematics, background work, staged reveals (`sceneBusy`)
**The report:** on phones the entrance drop and the end of the kidnap (the lock-in, when
the focus UI arrives) were the laggiest moments. Measured, a whole world frame is ~0.4 ms
of JS on a desktop — the drawing was never the problem. **Everything else chose the same
instant**: every "on idle after spawn" task fired in the idle gaps of the 30 fps cap, i.e.
during the entrance (the race track's pixel read alone is hundreds of ms on a phone), and
the lock-in did a dozen panel reveals + a forced layout + audio start-up + Firebase writes
(whose local listeners fire synchronously) in ONE frame.

- **`sceneBusy()`** = boot not lifted, entrance pending/active, kidnap (`anim`), sofa hop,
  or a staged reveal in progress. Four things respect it:
  1. **`whenCalm(fn)`** — one background task at a time, only after `BG_CALM_MS` of calm,
     `BG_GAP_MS` between tasks (an async task holds the queue until it settles), ceiling
     `BG_MAX_WAIT_MS`. `calmTurn()` is the promise form for a continuation, and does NOT
     go through the queue (a queued task awaiting it would deadlock).
  2. **The governor** ignores those frames and `PERF_SETTLE_MS` after them — a
     transition's cost is not the device's steady state, and a resolution step (a canvas
     realloc) in the middle of the moment it misjudged was a hitch of its own.
  3. **Sprite density** (`_sprUpdateLevel`): chosen once a frame with hysteresis, and from
     the entrance's TARGET zoom while it plays — the 0.8 → 1 zoom crossed a level and
     re-baked every name/badge/avatar mid-drop. Bakes are rationed (`SPR_BAKES_PER_FRAME`);
     past the ration another level's copy (or the live path) stands in for a frame.
  4. **`stageFrames(steps)`** — one step per rendered frame, drained by a watchdog if the
     loop stops (hidden tab). The kidnap's lock-in sets `gameState._lockInSteps = {ui, net}`
     around `startPomodoroPhase('work')` / `_startFreeModeWork()`; they queue their visible
     changes (focus mode → panels → audio) and their writes, net last. Every staged step
     re-checks the session is still in work (`_stillWork`) — a step must never resurrect
     a doc or re-show a panel after the session ended in those ~150 ms. Every OTHER caller
     of those two functions runs everything inline, as before.
- The kidnap's progress runs on real elapsed time (`dtAmbient`, capped at 4) — clamped
  `dtFactor` turned every long frame into slow motion on top of the dropped frame.
- The entrance's black fade is a CSS transition (compositor), not a per-frame style write.
- Dust is drawn batched (8 alpha bands, one path each) — the kidnap keeps ~100 alive.
...
- **الجسيمات / الطبقات الجوية live behind a «مخصّص» checkbox** (`#settings-effects-custom`, `role=checkbox` — NOT `settings-custom-btn`, that id is the HUD's تخصيص الشخصية button; reusing it wired this checkbox's handler onto that button). Unticked = both keys absent = follow the tier (the old تلقائي). Ticking it **pins what is showing right now** (writes both keys with the resolved values) and reveals two on/off pills (`.stg-reveal`, a `0fr ↔ 1fr` grid-rows transition). Unticking removes both keys. The storage and `particlesEnabled()` / `overlaysEnabled()` are unchanged.
...
the left column is **الخروج pill → tools box (crown, تخصيص, gear) → azkar dock**, each
hung off the bottom of the one above it; the right column keeps only the **user card →
حضور المقر**,
...
- **The left column is stacked from `top + offsetHeight`, never `getBoundingClientRect`.**
  ...
  measured by `offsetHeight` against its fixed CSS top, not by its rect.
...
- **Measure the BOX, never the drawn rect** (`_hudLayoutRect`): focus mode slides the
  ...
  That is how حضور المقر landed on the wrong spot at session end and then SNAPPED under
  the card when a 520 ms re-measure caught up. `_hudLayoutRect` subtracts the live
  transform's translation; the re-measure is gone, and `setMobileFocusMode` asks for one
  coalesced measure next frame (`_hudScheduleDock`) instead of forcing a layout mid-toggle.
...
- **Auto photo** (`_autoShot`, `_autoShotTick` in `SLOW_TASKS`): during a work phase, seated at the laptop, the world is rendered every `AUTO_SHOT_EVERY_MS` (first after 8 s) into a 640×480 offscreen canvas through `renderPiPInto(ctx, cv, 1, view)` (the `view` arg = own camera, no PiP chrome), framed on player + laptop, kept as a JPEG dataURL in memory. It must be taken DURING work — by the end card the session is torn down and free mode has stood the player up. A new session id (`_dutySessId`) resets it; an ended one (`''`) keeps it. On حفظ with the slot still in `'drop'` (not ✕-removed) `runSaveSequence` fades it in (`.auto-in`, `backwards` fill) + plays `sparkle` (`Sound/Sparkle.mp3`, 4-step pattern), waits `AUTO_SHOT_REVEAL_MS`, then `_runSaveSequenceMain` — so it is archived like a dropped photo.
...
8. **Verify** per the Operating Procedure §4: `node --input-type=module --check < game.js`, diff scanned
